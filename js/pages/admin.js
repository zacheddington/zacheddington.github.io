...
async function loadUsers() {
    try {
        ...
        const usersTableBody = document.getElementById('usersTableBody');
        if (usersTableBody) {
            usersTableBody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: #dc3545;">Error loading users. Please try again.</td></tr>';
        }
    } finally {
        const usersLoading = document.getElementById('usersLoading');
...
function editUserRole(userId) {
    ...
    window.modalManager.showModal('info', 'User role editing functionality will be implemented in a future update.');
}

function deleteUser(userId, username) {
    console.log('Delete user functionality for ID:', userId);
    window.modalManager.showModal('info', 'User deletion functionality will be implemented in a future update.');
}
