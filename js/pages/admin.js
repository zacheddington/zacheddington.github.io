 ...
 * Admin Page Module
 * @module pages/admin
 * @description Handles all admin functionality including:
 ...
 * @requires password-utils.js - Password validation
 * @requires field-validation.js - Form validation
 * @requires string-utils.js - String formatting utilities
 * @requires auth-utils.js - Authentication utilities
...
/**
 * Initialize admin page based on current route
...
async function checkUsernameAvailability(username) {
  ...
  try {
    ...
    const currentCheckingMsg = usernameGroup.querySelector(".checking-message");
  ...
  usersTableBody.innerHTML = users
    .map((user) => {
      ...
      const primaryRoleKey =
        user.role_keys && user.role_keys.length > 0 ? user.role_keys[0] : 2;
      ...
      if (user.date_created) {
        try {
          ...
          console.error("Date parsing error for user:", user.username, error);
          createdDate = "Date error";
        }
      }
      const isCurrentUser = currentUser.username === user.username;
      // Escape user data for safe HTML insertion (XSS prevention)
      const safeUsername = escapeHtml(user.username);
      const safeFullName = escapeHtml(fullName);
      const safeEmail = escapeHtml(user.email);
      return `
            <tr data-user-id="${user.user_key}">
                <td class="user-username" title="${safeUsername}">${safeUsername}</td>
                <td class="user-fullname" title="${safeFullName}">${safeFullName}</td>
                <td class="cell-email" title="${safeEmail}">${safeEmail}</td>                <td>
                    ${
                      currentRoles.length > 0
                        ...
                        : `<span class="user-role ${roleClass}" data-role-key="${primaryRoleKey}" title="${escapeHtml(
                            primaryRole
                          )}">
                ...
                <td class="cell-date" title="${createdDate}">${createdDate}</td>                <td>
                    <div class="table-actions">
                        ${
                          !isCurrentUser
                            ? `<button class="btn-icon btn-delete" onclick="window.adminPage.deleteUser(${
                                user.user_key
                              }, '${escapeJavaScript(
                                user.username
                              )}')" title="Delete User">
...
async function editUserRole(userId, newRoleKey) {
  ...
  if (user && currentUser.username === user.username) {
    window.modalManager.showModal("error", "You cannot change your own role.");
    // Reset the dropdown to its original value
    const roleSelect = document.querySelector(`select[onchange*="${userId}"]`);
    if (roleSelect) {
  ...
  if (newRoleKey !== undefined) {
    ...
    window.modalManager.showConfirmModal(
      ...
      () => {
        ...
        if (roleSelect && typeof roleSelect.revertToOriginal === "function") {
    ...
    if (response.ok) {
      ...
      setTimeout(async () => {
        try {
          ...
          console.warn("Failed to check current session validity:", checkError);
        }
      ...
      try {
        ...
        try {
          ...
          console.error("Could not parse server response:", textError);
        }
        ...
        switch (response.status) {
          ...
          case 404:
            ...
            break;
          case 403:
            errorMessage = "You do not have permission to revoke sessions.";
            break;
          case 500:
            errorMessage =
...
async function performBulkSessionRevocation(username) {
  try {
    ...
    const response = await fetch(
      `${API_URL}/api/sessions/revoke-user/${encodeURIComponent(username)}`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (response.ok) {
      ...
      if (window.modalManager) {
        window.modalManager.showModal(
          "success",
          `Revoked ${result.revokedCount || 0} sessions for user ${username}`
        );
      } else {
        alert(
          `Revoked ${result.revokedCount || 0} sessions for user ${username}`
      ...
      throw new Error(`Failed to revoke user sessions: ${response.status}`);
    }
  } catch (error) {
