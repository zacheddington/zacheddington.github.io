...
(function() {
    ...
    const headElements = {
        ...
        icons: [
            { rel: 'icon', type: 'image/x-icon', href: getFaviconPath('favicon.ico') },
            { rel: 'icon', type: 'image/svg+xml', href: getFaviconPath('favicon.svg') },
    ...
    function getFaviconPath(filename) {
        ...
        const isRootPage = path === '/' || path.endsWith('/index.html') || !path.includes('/');
    ...
    function addPreconnects() {
        headElements.preconnects.forEach(preconnect => {
            const existing = document.querySelector(`link[href="${preconnect.href}"]`);
            if (!existing) {
