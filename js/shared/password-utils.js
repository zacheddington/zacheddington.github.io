...
function validatePasswordStrength(password) {
  ...
  if (!requirements.hasUpperCase)
    failed.push("At least one uppercase letter (A-Z)");
  if (!requirements.hasLowerCase)
    failed.push("At least one lowercase letter (a-z)");
...
function calculatePasswordScore(password) {
  ...
  // Pattern diversity bonus (max 20)
  ...
  // Penalties (applied after bonuses, but score can still reach 100 if all requirements met)
  // Only penalize truly weak patterns, not common safe patterns like "123" in a strong password
  if (/(.)\1{3,}/.test(password)) score -= 10; // 4+ repeated characters (more lenient)
  if (/\s/.test(password)) score -= 20; // Spaces are disallowed
...
function updatePasswordStrength(password, inputId) {
  ...
  const strengthFill = document.querySelector(
    `#${inputId} + .password-strength-container .password-strength-fill`
  );
  const strengthText = document.querySelector(
    `#${inputId} + .password-strength-container .password-strength-text`
  );
  const requirementsList = document.querySelector(
    `#${inputId} + .password-strength-container .password-requirements`
  );
  ...
  if (requirementsList) {
    const requirementItems =
      requirementsList.querySelectorAll(".requirement-item");
...
function validatePasswordMatch(
  ...
  const existingMessage = confirmGroup.querySelector(
