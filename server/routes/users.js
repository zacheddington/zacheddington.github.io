...
const { authenticateToken, requireAdmin, preventSelfModification } = require('../middleware/auth');
...
router.get('/roles', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        return successResponse(res, rolesResult.rows, 'Roles retrieved successfully');
...
router.post('/create-user', 
    ...
    requireAdmin,
    sanitizeInput,
    validateRequiredFields(['firstName', 'lastName', 'email', 'username', 'password', 'roleKey']),
    ...
    async (req, res) => {
        try {
            ...
            if (!passwordValidation.isValid) {
                return errorResponse(res, 'Password does not meet security requirements', 400, passwordValidation.errors);
            }
            ...
            try {
                ...
                if (existingUserResult.rows.length > 0) {
                    ...
                    return conflictResponse(res, 'Username already exists. Please choose a different username.');
                }
                ...
                if (existingEmailResult.rows.length > 0) {
                    await client.query('ROLLBACK');
                    return conflictResponse(res, 'Email address is already registered. Please use a different email.');
                }
            ...
            return errorResponse(res, err.message || 'Failed to create user. Please try again later.', 500);
        }
    }
);
