...
router.post('/2fa/setup', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            await client.query(
                'UPDATE tbl_user SET twofa_secret = $1, date_when = NOW() WHERE user_key = $2',
            ...
            try {
                ...
                if (!user.twofa_secret) {
                    return errorResponse(
                        res,
                        '2FA not set up. Please run setup first.',
                        400
                    );
                }
                ...
                await client.query(
                    'UPDATE tbl_user SET twofa_enabled = true, backup_codes = $1, twofa_setup_date = NOW(), date_when = NOW() WHERE user_key = $2',
    ...
    async (req, res) => {
        try {
            ...
            try {
                ...
                await client.query(
                    'UPDATE tbl_user SET twofa_enabled = false, twofa_secret = NULL, backup_codes = NULL, date_when = NOW() WHERE user_key = $1',
                    [userId]
                );
