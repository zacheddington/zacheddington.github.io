...
router.post(
  ...
  async (req, res) => {
    try {
      const { firstName, middleName, lastName, address, phone, acceptsTexts } =
        req.body;
      ...
      return errorResponse(
        res,
        err.message || "Failed to create patient. Please try again later.",
        500
      );
    }
  }
...
router.delete("/patients/:patientKey", authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      await client.query("DELETE FROM tbl_patient WHERE patient_key = $1", [
        patientKey,
      ]);

      // Delete associated name data if it exists
      if (nameKey) {
        await client.query("DELETE FROM tbl_name_data WHERE name_key = $1", [
          nameKey,
        ]);
      }
